...
import (
	...
	"github.com/satyapraneel/goweb/pkg/arrays"
)
...
func (f *Form) PermittedValues(field string, opts ...string) {
	...
	exists := arrays.InArray(value, opts)
	if exists {
		return
	}
...
func (f *Form) MinLength(field string, d int) {
	...
	if utf8.RuneCountInString(value) < d {
		f.Errors.Add(field, fmt.Sprintf("This field length is too short (maximum is %d characters)", d))
	}
}
